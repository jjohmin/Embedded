...
#define WIFI_SSID ".."
#define WIFI_PASSWORD "jhmin7624"

#define FIREBASE_HOST "led-onoff-abf9e-default-rtdb.firebaseio.com"

#define FIREBASE_AUTH "XJa0PFzmHbVh4RQe5oFnLfqsQVDlXk7Dsnto25gT"
...
void loop() {
  if (Serial.available()) {
    int ch = Serial.read();
    if (ch == '1') {
      // LED를 ON으로 설정하고 Firebase에 값을 기록합니다.
      if (Firebase.setString(fbdo, "LED", "ON")) {
        Serial.println("LED ON");
      } else {
        Serial.println("FAILED");
      }
    } else if (ch == '2') {
      // LED를 OFF로 설정하고 Firebase에 값을 기록합니다.
      if (Firebase.setString(fbdo, "LED", "OFF")) {
        Serial.println("LED OFF");
      } else {
        Serial.println("FAILED");
      }
    } else if (ch == '3') {
      if (Firebase.get(fbdo, "LED")) {
        if (fbdo.dataType() == "string") {
          String value = fbdo.stringData();
          Serial.println("Read data: " + value);
        }
      }
    }
  }
